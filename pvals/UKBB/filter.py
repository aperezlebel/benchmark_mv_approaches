...
def filter():
    pvals_dirs = next(os.walk('pvals/UKBB/'))[1]
    ...
    for category in categories:
        with open(f'pvals/UKBB/categories/{category}.txt', 'r') as file:
    ...
    for pvals_dir in tqdm(pvals_dirs):
        for filename in next(os.walk(f'pvals/UKBB/{pvals_dir}'))[2]:
            if 'pvals.csv' not in filename:
                continue  # not pvals, skipping

            pvals_path = f'pvals/UKBB/{pvals_dir}/{filename}'
