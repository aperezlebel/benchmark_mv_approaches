...
for method in [20, 24, 22, 26]:
    for task in tasks:
        ...
        for T in range(5):
            ...
            command = f"salloc --ntasks 1 --cpus-per-task {args.n_cpus} --job-name {method}{T}{db[0]}{name}{partition_option}{time_option} srun --pty python main.py predict {task} {method} --RS 0 --T {T} --nbagging 100{train_size_option}"
